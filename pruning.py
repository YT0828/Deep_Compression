...
parser.add_argument('--re-epochs', type=int, default=5, metavar='N',
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in pbar:
            ...
            for name, p in model.named_parameters():
                ...
                #################################
                if name in ['fc1.weight', 'fc2.weight', 'fc3.weight', 'conv1.weight', 'conv2.weight', 'conv3.weight', 'conv4.weight', 'conv5.weight']:
                    weights = p.data
                    grad = p.grad.data
                    # print('name:', name)
                    # print('grad data size:', grad.size())
                    # print('weight data size:', weights.size())
                    # print('grad before:', p.grad.data)
                    mask = weights.ne(0)
                    val = torch.zeros(grad.size(), device = device)        
                    val[mask] = grad[mask]
                    p.grad.data = val
...
def main():
    ...
    # -------------------------------
    model = torch.load(args.load_model_path, map_location=torch.device(device))
    accuracy = test()
    util.log(f"Initial_accuracy {accuracy}", args.log)
    util.print_nonzeros(model)
