...
def huffman_encode_conv(param, name, directory):
    ...
    # We need four lists: "data", "length of data", "indices", and "indptr"
    indptr_list = []
    indices_list = []
    data_list = []
    data_len_list = []
    # try onlu using csr first
    form = 'csr'
    for Kn in range(shape[0]):
        for Ch in range(shape[1]):
            mat = csr_matrix(weight[Kn][Ch])
            data_list.extend(mat.data)
            indices_list.extend(mat.indices)
            indptr_list.extend(calc_index_diff(mat.indptr))
            data_len_list.append(len(mat.data))

    #print(np.array(data_list))
    # Encode

    t0, d0 = huffman_encode(np.array(data_list, dtype = 'float32'), name + f'_{form}_data', directory)
    t1, d1 = huffman_encode(np.array(indices_list, dtype = 'int32'), name + f'_{form}_indices', directory)
    t2, d2 = huffman_encode(np.array(indptr_list, dtype = 'int32'), name + f'_{form}_indptr', directory)
    t3, d3 = huffman_encode(np.array(data_len_list, dtype = 'int32'), name + f'_{form}_data_len', directory)

    # Print statistics
    original = param.data.cpu().numpy().nbytes
    compressed = t0 + t1 + t2 + d0 + d1 + d2 + t3 + d3
    log_text = (
...
def huffman_decode_conv(param, name, directory):
    ...
    # only consider csr here
    form = 'csr'
    matrix = csr_matrix

    # Decode data
    data_list = huffman_decode(directory, name + f'_{form}_data', dtype='float32')
    data_len_list = huffman_decode(directory, name + f'_{form}_data_len', dtype='int32')
    indices_list = huffman_decode(directory, name + f'_{form}_indices', dtype='int32')
    indptr_list = huffman_decode(directory, name + f'_{form}_indptr', dtype='int32')
    ...
    for Kn in range(shape[0]):
        for Ch in range(shape[1]):
            data_len = data_len_list[data_len_idx]
            data = data_list[start_idx : (start_idx + data_len)]
            indices = indices_list[start_idx : (start_idx + data_len)]
            indptr = reconstruct_indptr(indptr_list[indptr_idx : (indptr_idx + shape[2])])
            mat = matrix((data, indices, indptr), shape = shape[2:])
            weight[Kn,Ch,:,:] = mat.toarray()
            data_len_idx+=1
            start_idx += data_len
            indptr_idx += shape[2]
